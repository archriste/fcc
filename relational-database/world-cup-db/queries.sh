...
echo  "$($PSQL "SELECT distinct(name) FROM games AS win FULL JOIN teams ON win.winner_id=teams.team_id FULL JOIN games AS opp ON opp.opponent_id=teams.team_id WHERE win.year=2014 OR opp.year=2014 ORDER BY name")"
